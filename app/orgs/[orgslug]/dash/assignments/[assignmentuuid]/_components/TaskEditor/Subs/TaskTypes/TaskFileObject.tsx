...
export default function TaskFileObject({ view, user_id, assignmentTaskUUID }: TaskFileObjectProps) {
    ...
    return (
        <AssignmentBoxUI submitFC={submitFC} showSavingDisclaimer={showSavingDisclaimer} view={view} gradeCustomFC={gradeCustomFC} currentPoints={userSubmissionObject?.grade} maxPoints={assignmentTaskOutsideProvider?.max_grade_value} type="file">
            {view === 'teacher' && (
                <div className='flex flex-col sm:flex-row py-5 sm:py-6 text-xs sm:text-sm justify-center mx-auto space-y-2 sm:space-y-0 sm:space-x-3 text-purple-300 px-4 sm:px-2 text-center sm:text-left bg-slate-900/40 backdrop-blur-xl border border-purple-500/30 rounded-lg shadow-lg shadow-purple-500/10'>
                    <Info size={18} className="mx-auto sm:mx-0 text-purple-400" />
            ...
            {view === 'custom-grading' && (
                <div className='flex flex-col space-y-4 w-full px-2 sm:px-0'>
                    <div className='flex flex-col sm:flex-row py-5 sm:py-6 text-xs sm:text-sm justify-center mx-auto space-y-2 sm:space-y-0 sm:space-x-3 text-purple-300 px-4 sm:px-2 text-center sm:text-left bg-slate-900/40 backdrop-blur-xl border border-purple-500/30 rounded-lg shadow-lg shadow-purple-500/10'>
                        <Download size={18} className="mx-auto sm:mx-0 text-purple-400" />
                    ...
                    {userSubmissions.fileUUID && !isLoading && assignmentTaskUUID && (
                        <Link
                            ...
                            target='_blank'
                            className='flex flex-col rounded-lg bg-black/20 backdrop-blur-xl border border-purple-500/30 text-purple-300 shadow-lg shadow-purple-500/10 hover:shadow-purple-500/20 transition-all px-4 sm:px-5 py-4 space-y-1 items-center relative w-full sm:w-auto mx-auto'>
                            <div className='absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2 bg-emerald-500 rounded-full p-1.5 text-white flex justify-center items-center shadow-lg'>
                                <Cloud size={14} />
                            </div>
            ...
            {view === 'student' && (
                <>
                    <div className="w-full bg-black/20 backdrop-blur-xl border border-purple-500/30 rounded-lg min-h-[200px] shadow-lg shadow-purple-500/10 px-4 sm:px-6 py-5 sm:py-6">
                        <div className="flex flex-col justify-center items-center h-full w-full">
                            <div className="flex flex-col justify-center items-center w-full max-w-full">
                                <div className="flex flex-col justify-center items-center w-full">
                                    {error && (
                                        <div className="flex justify-center bg-rose-600/20 backdrop-blur-xl border border-rose-400/30 rounded-md text-rose-300 space-x-2 items-center p-3 transition-all shadow-lg shadow-rose-500/20 w-full sm:w-auto mb-4">
                                            <div className="text-xs sm:text-sm font-medium">{error}</div>
                                        </div>
                                    )}
                                </div>
                                {localUploadFile && !isLoading && (
                                    <div className='flex flex-col rounded-lg bg-slate-900/40 backdrop-blur-xl border border-purple-500/30 text-purple-300 shadow-lg shadow-purple-500/10 px-4 sm:px-5 py-4 space-y-1 items-center relative w-full sm:w-auto mt-3'>
                                        <div className='absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2 bg-emerald-500 rounded-full p-1.5 text-white flex justify-center items-center shadow-lg'>
                                            <Cloud size={14} />
                                        </div>
                                ...
                                <div className='flex flex-col sm:flex-row pt-5 font-medium space-y-1 sm:space-y-0 sm:space-x-2 text-xs items-center text-purple-300 text-center sm:text-left bg-slate-900/40 backdrop-blur-xl border border-purple-500/30 rounded-lg px-3 py-2 mt-5 shadow-lg shadow-purple-500/10 w-full sm:w-auto'>
                                    <Info size={15} className="mx-auto sm:mx-0 text-purple-400" />
                                    <p>Allowed formats: pdf, docx, mp4, jpg, jpeg, png, pptx, zip</p>
                                </div>
                                {!access_token ? (
                                    <div className="flex justify-center items-center w-full mt-5">
                                        <div className="flex justify-center bg-amber-600/20 backdrop-blur-xl border border-amber-400/30 rounded-md text-amber-300 space-x-2 items-center p-3 transition-all shadow-lg shadow-amber-500/20 w-full sm:w-auto">
                                    ...
                                    <div className="flex justify-center items-center w-full mt-5">
                                        ...
                                        />
                                        <div className="font-medium animate-pulse antialiased items-center bg-slate-700/60 backdrop-blur-xl border border-slate-500/30 text-slate-300 text-xs sm:text-sm rounded-md px-4 sm:px-5 py-2.5 flex shadow-lg">
                                    ...
                                    <div className="flex justify-center items-center w-full mt-5">
                                        ...
                                        />
                                        <button
                                            className="font-medium antialiased items-center text-white text-xs sm:text-sm rounded-md px-4 sm:px-5 py-2.5 flex bg-emerald-600/90 backdrop-blur-xl border border-emerald-400/30 shadow-lg shadow-emerald-500/20 hover:bg-emerald-600/70 transition-all"
